...
public class Calculator extends VBox implements EventHandler<ActionEvent>{
	...
	public void handle(ActionEvent event) {
		//ArrayList<String> nums = new ArrayList<String>(["1","2"]);
		...
		if (Arrays.stream(nums).toList().contains(value)){
			if(number1.equals("")) {
				number1 = value;
			} else if (number2.equals("")) {
				number2 = value;
			}
		} else if (Arrays.stream(ops).toList().contains(value) && operator.equals("")) {
			operator = value;
		}
		...
		if(value.equals("=")) {
			if (!(number1.equals("") || operator.equals("") || number2.equals(""))) {
				...
				displayText.setText(Float.toString(sol));
			}
